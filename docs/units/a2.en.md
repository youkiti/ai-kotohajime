...
5. Notice that the diagram code you generated here was, in fact, produced not by a diffusion model but by the same "predict the next word" mechanism as text-generation AI. Generating an actual image -- a photo or an illustration -- widely uses the diffusion mechanism described above (some newer systems generate images with the same predictive approach as an LLM), but that hands-on experience is outside the scope of this unit (see Core Concepts).
