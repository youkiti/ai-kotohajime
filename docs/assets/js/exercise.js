...
(function () {
  ...
  function nextStepText(area) {
    if (AIK && AIK.passedAt(area)) {
      return "下の「目録の発行」に進んでください。";
    }
    return "領域クイズの合格もそろうと、目録を発行できます。";
  }
  ...
  function renderForm(container, area, marker, minLength) {
    ...
    var label = el("label", { className: "aik-exercise-label", text: "AIの講評を貼り付けてください" });
    var textarea = document.createElement("textarea");
    textarea.className = "aik-exercise-textarea";
    textarea.setAttribute("rows", "10");
    textarea.placeholder = marker + " で始まる講評の全文を、ここに貼り付けます。";
    ...
    submitBtn.addEventListener("click", function () {
      ...
      if (text.indexOf(marker) === -1) {
        showError(
          "見出し「" +
            marker +
            "」が見つかりません。AIの講評を、1行目の見出しを含めて全文コピーして貼り付けてください。"
        );
        return;
      }
      if (text.length < minLength) {
        showError("テキストが短すぎるようです。講評の全文(見出しから改善版プロンプトの提案まで)を貼り付けてください。");
        return;
      }
