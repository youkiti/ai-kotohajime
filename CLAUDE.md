...
**試験公開版(β版)として公開中(A・B・C領域が日英とも実動)。** 全ページの骨格があり、A領域(Unit A-1/A-2 本文・領域Aクイズ10問・目録)、B領域(Unit B-1/B-2 本文・領域Bクイズ5問・実践課題の提出・目録)、C領域(Unit C-1/C-2 本文・領域Cクイズ10問・目録)が日本語版・英語版(`.en.md`/`.en.json`)の両方で動作する。全6ユニットに本文と自己チェッククイズがあり、プレースホルダは残っていない。

正本ドキュメント(実装より優先。仕様変更はまずここに反映):
- `documents/AI事始-要件定義書.md` — 要件・技術構成・MVP/試験公開版(β)定義・英語版(i18n)仕様
- `documents/AI事始-初級コンピテンシー.md` — 学習目標(A/B/C領域)とユニット構成
- `documents/AI事始-参考資料-JSME-ICTシンポジウム.md` — 外部参考資料の対応表の正本(リンクのみ掲載・転載禁止・確認日を記録)
- `documents/AI事始-作問ガイドライン.md` — クイズ選択肢の作問原則の正本(testwiseness cue 対策。長さ・語彙・構造・正答分散等の基準と lint 閾値を規定)
...
- B領域は職種別(医師/看護/薬学/リハ)タブで題材のみ差し替え、構造・クイズ・採点プロンプトは共通
...
- documents/ には現行版のみを置き、ファイル名にバージョンを含めない。バージョンは各文書冒頭の見出しと改訂履歴で管理し、改訂時は同じファイルを編集して改訂履歴に変更点を追記する。旧版は git 履歴で参照(`git log --follow`)
